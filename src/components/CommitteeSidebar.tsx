...
import { ChartBar, MessageSquare, RotateCcw, Star, Users } from 'lucide-react';
                    ...
                    >
                        ...
                        <div
                            ...
                            className={cn(
                                ...
                                isActive ? (result.status === 'error' ? "bg-red-50" : color.bg) : "bg-white hover:bg-slate-50"
                            )}
                        >
                            <div className="flex-1">
                                <h3 className={cn("font-bold", isActive ? (result.status === 'error' ? "text-red-700" : color.text) : "text-slate-700")}>
                                    {result.schoolName.replace(/\s*\(.*?\)\s*$/, '').trim()}
                                </h3>
                                {result.status === 'error' ? (
                            ...
                            <div className={cn(
                                "w-10 h-10 rounded-full flex items-center justify-center font-serif font-bold text-lg shrink-0 ml-4",
                                isActive
                                    ? (result.status === 'error' ? "bg-white text-red-500 border border-red-100" : "bg-white text-primary")
                                    : "bg-slate-50 text-slate-400"
                            )}>
                                {result.status === 'error' ? "!" : Math.round(result.scores.overall)}
                            </div>
                        </div>
                                ...
                                {result.status === 'error' ? (
                                    <div className="space-y-4 py-4 text-center">
                                        <div className="p-4 bg-red-50 rounded-xl border border-red-100 text-red-700 text-sm">
                                            <p className="font-medium mb-1">Error during simulation</p>
                                            <p className="opacity-80 text-xs">{result.error}</p>
                                        </div>
                                        <button
                                            onClick={() => onRetry?.(result.schoolName)}
                                            className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors"
                                        >
                                            <RotateCcw className="w-4 h-4" /> Retry School
                                        </button>
                                        <p className="text-[10px] text-slate-400">This retry will not consume an extra credit.</p>
                                    </div>
                                ) : (
                                    <>
                                        {/* Reaction */}
